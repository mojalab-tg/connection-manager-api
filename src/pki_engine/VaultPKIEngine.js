...
class VaultPKIEngine extends PKIEngine {
    constructor({
        ...
        this.trustedCaStore = forge.pki.createCaStore(tls.rootCertificates.filter(cert => {
            try { forge.pki.certificateFromPem(cert); } catch { return false; }
            return true;
        }));
    }
  ...
  async createCSR (csrParameters) {
    ...
    async sign(csr, commonName) {
        ...
        console.log("pkiClientRole", this.pkiClientRole);
        console.log("this.mounts.pki", this.mounts.pki);
        const { data } = await this.client.request({
...
*/
    ...
    static _getExtensionsInfo(forgeCert) {
        ...
        forgeCert?.extensions?.find(t => t.name === 'subjectAltName')?.altNames
            .forEach(item => {
                switch (item.type) {
